...
# This file is part of chizuru_mizuhara (Telegram Bot)
...
from chizuru_mizuharaBot import BOT_ID
from chizuru_mizuharaBot.conf import get_int_key, get_str_key


from chizuru_mizuharaBot.pyrogramee.telethonbasics import is_admin
from chizuru_mizuharaBot.events import register
from chizuru_mizuharaBot import MONGO_DB_URI 
from pymongo import MongoClient
from chizuru_mizuharaBot.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
...
async def ws(event):
    ...
    if is_nsfw == True:
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n[{st}](tg://user?id={hh}) your message contain NSFW content.. So, chizuru_mizuhara deleted the message\n\n **Nsfw Sender - User / Bot :** [{st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By chizuru_mizuharaAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, chizuru_mizuhara deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By chizuru_mizuharaAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
...
__help__ = """
*Group Guardian:*
✪ chizuru_mizuhara can protect your group from NSFW senders, Slag word users and also can force members to use English
